...
First, we make a trivial observation: that binary vectors are corners of the unit hypercube. Let's assume we are in $R^3$, then we have (unsurprisingly) the following corners (excluding the origin):
